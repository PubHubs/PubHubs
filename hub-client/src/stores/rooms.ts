...
const useRooms = defineStore('rooms', {
	...
	actions: {
		...
		async joinRoomListRoom(roomId: string) {
			...
			if (!this.rooms[roomId]) {
				// Check if user is a member before trying to join
				// This prevents errors for rooms the user has left (can't rejoin private rooms)
				// Skip silently if not a member - don't hide the room as it might affect other users
				if (user.userId && !(await pubhubs.isUserRoomMember(user.userId, roomId))) {
					return;
				}
			...
			if (!lastReadMessageId && room) {
				lastReadMessageId = (await pubhubs.client.createMessagesRequest(roomId, null, 1, Direction.Backward, room.getMessagesFilter())).chunk[0]
					?.event_id;
			}
			if (lastReadMessageId && room) {
