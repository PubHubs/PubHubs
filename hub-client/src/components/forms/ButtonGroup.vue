<template>
    <div class="flex">
        <Button v-for="(option,index) in options" :key="index" :color="optionIsSelected(option)?'blue':'gray-light'" :size="size" :class="roundedClass(index)" @click="selectOption(option);changed();">{{ option.label }}</Button>
    </div>
</template>
