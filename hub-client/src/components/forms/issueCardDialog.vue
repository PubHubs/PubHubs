<template>
	<Dialog :title="cardType === 'yellow' ? capitalize($t('moderation.issue_yellow_card')) : capitalize($t('moderation.issue_red_card'))" :width="isMobile ? 'px-8 w-full' : 'w-[600px] px-8'" @close="close()" class="z-50">
		<ValidatedForm @submit.prevent class="flex flex-col p-200" :class="isMobile ? 'w-full' : 'w-[450px]'" v-slot="{ isValidated }">
			...
			<TextArea :validation="{ required: true, maxLength: 500 }" v-model="reason" @keydown.esc.stop>{{ capitalize($t('moderation.card_reason_label')) }}</TextArea>
