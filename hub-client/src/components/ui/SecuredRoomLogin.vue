<template>
	<div class="absolute top-16 flex flex-col text-center">
		...
		<div class="z-20 box-border min-h-[333px] w-[255px] rounded-lg bg-white p-0 drop-shadow-[0px_-5px_16px_rgb(0,0,0,0.15)]">
			...
			<div
				v-if="!loginFlow"
				class="mx-4 my-12 flex flex-col gap-12 after:absolute after:-top-[1.2em] after:left-[50%] after:border-[1.25em] after:border-t-0 after:border-r-0 after:border-transparent after:border-b-white after:drop-shadow-[0px_-5px_16px_rgb(0,0,0,0.15)]"
			>
				<div class="flex flex-row items-center">
					<Icon type="lock" size="md" />
					<P class="text-label ml-2 text-left break-words whitespace-pre-wrap dark:text-black">{{ t('rooms.secure_room_message') }}</P>
				</div>
				<P class="text-body text-gray dark:text-gray-middle max-h-24 w-full overflow-y-auto break-words whitespace-pre-wrap" :title="rooms.securedRoom.user_txt">{{ rooms.securedRoom.user_txt }}</P>
