<template>
	<div
		:id="topic.eventId"
		class="bg-surface m-5 space-y-1 rounded-xl px-4 py-2"
	>
		<div class="mb-2 flex flex-row items-center">
			<AvatarId
				v-if="roomMember"
				:user-id="roomMember.userId"
			></AvatarId>
				...
				<div class="flex w-full items-center justify-between gap-2">
					<span class="flex items-center gap-1">
						<UserDisplayName
							...
							:user-display-name="topic.author?.displayName"
						></UserDisplayName>
						<EventTime
					...
					>
						<ActionMenu>
							...
							<ActionMenuItem
								v-if="currentUserIsTopicAuthor && !props.topic.closed && !props.replies"
								@click="closeOrOpenTopic(true)"
								>Close</ActionMenuItem
							>
							<ActionMenuItem
								v-if="currentUserIsTopicAuthor && props.topic.closed && !props.replies"
								@click="closeOrOpenTopic(false)"
								>Open</ActionMenuItem
							>
							<ActionMenuItem
								v-if="currentUserIsTopicAuthor"
								@click="OnDeleteClick"
								>Delete</ActionMenuItem
							>
							<ActionMenuItem
								v-if="currentUserIsTopicAuthor"
								@click="toggleEdit(topic.eventId)"
								>Edit</ActionMenuItem
							>
						</ActionMenu>
					</div>
		...
		<EditTopicInput
			...
			@submit="toggleEdit(topic.eventId)"
		/>
		<LabelWithDescription
			v-else
			show
			class="space-y-1"
			description-class="text-black text-justify dark:text-white"
		>
			{{ topic.title }}
			<template #description>
				<div
					v-if="topic.image || topic.file"
					class="p-4 pl-0"
				>
					<div class="min-w-40">
						<MessageImage
							v-if="topic.image"
							:message="topic.image.content"
						/>
						<MessageFile
							v-if="topic.file"
							:message="topic.file.content"
						/>
					</div>
				</div>
				{{ topic.body }}
		...
		<div class="flex w-full flex-row gap-x-4">
			<div
				class="flex cursor-pointer flex-row items-center select-none"
				@click="handleRating(UserInteraction.LIKED)"
			>
				<Icon
					type="thumbs-up"
					size="sm"
					:filled="interaction === UserInteraction.LIKED"
					:class="interaction === UserInteraction.LIKED ? 'text-green-900' : ''"
				/>
				<span
					class="ml-1"
					:class="interaction === UserInteraction.LIKED ? 'text-white' : 'text-gray-light'"
					>{{ localLikes }}</span
				>
			</div>
			...
			<div
				class="flex cursor-pointer flex-row items-center select-none"
				@click="handleRating(UserInteraction.DISLIKED)"
			>
				<Icon
					type="thumbs-down"
					size="sm"
					:filled="interaction === UserInteraction.DISLIKED"
					:class="interaction === UserInteraction.DISLIKED ? 'text-red-900' : ''"
				/>
				<span
					class="ml-1"
					:class="interaction === UserInteraction.DISLIKED ? 'text-white' : 'text-gray-light'"
					>{{ localDisikes }}</span
				>
			</div>

			<span
				v-if="mainTopic.closed === false"
				class="ml-1 cursor-pointer select-none"
				:class="isActiveReplyTopic ? 'text-white' : 'text-gray-light'"
				@click="toggleReply(topic.eventId)"
				>Reply</span
			>

			<div
				v-if="replies && localReplies.length !== 0"
				class="flex cursor-pointer flex-row items-center select-none"
			...
			>
				<Icon
					...
					size="sm"
					:class="showReplies ? '' : 'text-gray-light'"
				/>
				<span
					class="mr-1 ml-1"
					:class="showReplies ? '' : 'text-gray-light'"
					>{{ localReplies.length }}</span
				>
				<Icon
					:type="showReplies ? 'arrow-down' : 'arrow-up'"
					size="sm"
					:class="showReplies ? '' : 'text-gray-light'"
				/>
			</div>
	...
	<ForumInput
		v-if="isActiveReplyTopic"
		class="m-5"
		:min_length="REPLY_MIN_LENGTH"
...
<script setup lang="ts">
	...
	import { type TTopicReplyContent } from '@hub-client/models/events/forum/TTopicEvent';
	// Models
	...
	// Stores
	import { type User } from '@hub-client/stores/user';
	...
	async function submitReply(payload: { text: string; image: TLocalAttachmentMessageEventContent | null; file: TLocalAttachmentMessageEventContent | null }) {
		try {
			...
			if (!reply) return logger.error('Failed to send reply');
			...
			logger.error('error in submitting reply', { error });
		} finally {
