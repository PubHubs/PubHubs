...
<script setup lang="ts">
	...
	const searchResultsToShow = computed(() => {
		...
		const filteredSearchResults = searchResults.value.filter((result) => {
			return result.event_body !== '';
		});
	...
	function mapSearchResult(results: SearchResult[]): TSearchResult[] {
		...
		let mappedResults = results
			.filter((result) => !('m.new_content' in result.context.ourEvent.event.content!))
			.map(
				(result) =>
					({
						rank: result.rank,
						...
						event_body: result.context.ourEvent.getContent().body,
						event_sender: result.context.ourEvent.event.sender,
					}) as TSearchResult,
			);
		mappedResults.forEach((element) => {
			element.event_body = formatSearchResult(element.event_body, value.value as string, maxNumberOfWordsInSearchResult);
		});
