	...
	<Dialog :title="title" :buttons="buttonsSubmitCancel" @close="close($event)" width="w-full max-w-[960px]">
		<form @submit.prevent class="flex flex-col gap-4">
			<FormLine>
				...
				<TextInput :placeholder="$t('admin.name')" v-model="editRoom.name" class="md:w-5/6" @changed="updateData('name', $event)" />
			</FormLine>
			<FormLine>
				<Label>{{ $t('admin.topic') }}</Label>
				<TextInput :placeholder="$t('admin.topic')" v-model="editRoom.topic" class="md:w-5/6" @changed="updateData('topic', $event)" />
			</FormLine>
			<FormLine v-if="!secured">
				<Label>{{ $t('admin.room_type') }}</Label>
				<TextInput :placeholder="$t('admin.room_type_placeholder')" v-model="editRoom.room_type" :disabled="!isNewRoom" class="md:w-5/6" @changed="updateData('room_type', $event)" />
			</FormLine>
			<div v-if="secured">
				<FormLine class="mb-2">
					<Label>{{ $t('admin.secured_description') }}</Label>
					<TextInput :placeholder="$t('admin.secured_description')" v-model="editRoom.user_txt" class="md:w-5/6" />
				</FormLine>
				<FormLine>
