<template>
	<div class="flex cursor-pointer items-center justify-start gap-4" @click="toggle()">
		<div class="inline-flex h-6 w-6 flex-col items-center justify-center gap-2">
			<div v-if="modelValue" class="bg-surface-base border-surface-on-surface-dim h-4 w-4 rounded border-[0.50px]"></div>
			<div v-else class="bg-button-on-blue outline-accent-blue flex h-4 w-4 flex-col items-center justify-center rounded p-0.5 outline-1 outline-offset-[-0.50px]">
		...
		<div class="inline-flex cursor-pointer flex-col items-center justify-center gap-2 pt-0.5">
...
<script setup lang="ts">
	...
	const { update, cancel, setValue } = useFormInputEvents(emit, props.modelValue);
