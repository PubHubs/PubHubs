<template>
	...
	>
		...
		<ValidatedForm
			v-if="roomLoaded"
			class="max-w-7000 px-400 py-200"
			:disabled="waitForServer"
			@keydown.enter.stop
			@validated="(v: boolean) => (isFormValidated = v)"
		>
			<TextField
				v-model="editRoom.name"
				:placeholder="t('admin.name_placeholder')"
				:show-length="true"
				:validation="{ required: true, maxLength: roomValidations.maxNameLength }"
			>
				{{ t('admin.name') }}
			</TextField>
			<TextField
				...
				:show-length="true"
				:validation="{ maxLength: roomValidations.maxTopicLength }"
			>
				{{ t('admin.topic') }}
			</TextField>
			<TextField
			...
			>
				<TextField
					...
					:show-length="true"
					:validation="{ required: true, maxLength: roomValidations.maxDescriptionLength }"
				>
					...
					>
						...
						<div class="flex flex-wrap gap-200">
							<Tabs v-slot="{ activeTab, setActiveTab }">
								...
								<TabContainer>
									<div
										v-if="selectedAttributes[activeTab]"
									...
									>
										...
										>
											...
											<div
												v-if="selectedAttributes[activeTab].accepted.length > 0"
												class="flex max-h-2000 w-full flex-wrap justify-start gap-100 overflow-y-scroll"
											>
												<span
													v-for="(value, index) in selectedAttributes[activeTab].accepted"
													:key="index"
													class="group bg-surface text-on-primary py-050 gap-050 inline-flex items-center rounded-xl px-100"
												>
													{{ value }}
													<button
														type="button"
...
<script lang="ts" setup>
	...
	onBeforeMount(async () => {
		...
		if (!isNewRoom.value) {
			...
			if (isSecured.value) {
				const accepted = (editRoom.value as TSecuredRoom).accepted ?? {};
				const [labels, attributes] = editRoomComposable.getYiviLabelsAndAttributes(accepted, t);
				selectedAttributes.value = JSON.parse(JSON.stringify(editRoomComposable.fillInEditFormAttributes(labels, attributes, accepted)));
				_OriginalAttributes = JSON.parse(JSON.stringify(selectedAttributes.value));
			}
		}
	});
	...
	async function submitRoom() {
		if (!isSecured.value) {
			...
			try {
				waitForServer.value = true;
				await editRoomComposable.updateSecuredRoom(
					isNewRoom.value,
					editRoom.value as unknown as TSecuredRoom,
