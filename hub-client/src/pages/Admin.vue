							...
							<div class="flex w-full justify-between gap-8 overflow-hidden" :title="item.room_id">
								...
								<div class="flex w-fit gap-4">
									...
									<div class="flex items-center gap-1">
										...
										<Icon type="edit" class="hover:cursor-pointer hover:text-accent-primary" v-if="rooms.room(item.room_id)?.userCanChangeName(user.userId)" @click="editPublicRoom(item)" />
										<Icon v-else type="promote" class="hover:cursor-pointer hover:text-accent-primary" @click="makeRoomAdmin(item.room_id, user.userId)" />
									</div>
								</div>
							</div>
						</template>
						...
						<template #item="{ item }">
							<div class="flex w-full justify-between gap-8 overflow-hidden" :title="item.room_id">
								...
								<div class="flex w-fit gap-4">
									...
									<div class="flex items-center gap-1">
										<Icon type="remove" class="hover:cursor-pointer hover:text-accent-red" v-if="rooms.room(item.room_id)?.userCanChangeName(user.userId)" @click="removeSecuredRoom(item)" />
										<Icon type="edit" class="hover:cursor-pointer hover:text-accent-primary" v-if="rooms.room(item.room_id)?.userCanChangeName(user.userId)" @click="EditSecuredRoom(item)" />
										<Icon type="promote" class="hover:cursor-pointer hover:text-accent-primary" @click="makeRoomAdmin(item.room_id, user.userId)" />
									</div>
								</div>
							</div>
						</template>
