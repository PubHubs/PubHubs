<template>
	<div class="mx-auto mt-20 flex w-6/12 flex-col gap-2 text-center">
		...
		<HubIcon v-if="hubName" :hub-name="hubName" :icon-url="hubSettings.iconUrlLight" :icon-url-dark="hubSettings.iconUrlDark" class="mx-auto max-h-20 max-w-24" />
		<P class="mb-2">{{ t('onboarding.info_first_time') }}</P>
		<P class="mb-2">{{ t('onboarding.info_abt_pseudonym') }}</P>
		<P class="text-green mb-0 text-2xl">{{ pseudonym }}</P>
		<P class="mb-2">{{ t('onboarding.info_issue_identity') }}</P>

		<TextInput v-if="!submitted" class="focus:border-blue mb-2 rounded border p-2 text-center focus:outline-none" placeholder="example: PubHubs" @changed="updateData($event)" />
		<P class="mb-2 ~text-label-min/label-max">{{ t('onboarding.info_abt_choose_later') }}</P>
		<P class="mb-2">{{ t('onboarding.info_abt_nickname_use') }}</P>
		<P class="mb-4">{{ t('onboarding.info_abt_yivi_room') }}</P>
		<P class="mb-2 ~text-label-min/label-max">{{ t('onboarding.info_misbehave') }}</P>
		<Button v-if="!submitted" class="mx-24" @click="submitAndClose">{{ t('onboarding.continue') }}</Button>
		<div v-if="submitted">
			<P class="text-green mb-0 text-2xl">{{ t('onboarding.update') }}!</P>
		</div>
	</div>
