<template>
	<div v-if="user.isLoggedIn && setupReady" class="bg-background font-body text-on-surface text-body flex h-screen w-full overflow-hidden">
		<HeaderFooter class="relative shrink-0" :class="isMobile && !hubSettings.isSolo && !hubSettings.isSolo ? 'w-[calc(50%-40px)]!' : 'flex max-w-4000'">
			<template #header>
		...
		<div class="h-full min-w-0 flex-1 overflow-hidden" :class="isMobile && !hubSettings.isSolo ? 'w-screen' : ''" role="document">
			<router-view />
		</div>
