	...
	<div class="p-200">
		...
		<div class="flex gap-200">
			<div class="border-spacing-200 rounded-lg border border-dotted border-purple-500 p-200">
				...
				<div class="gap-050 flex flex-col">
					...
					<Button
						...
						secondary-icon="smiley"
						@click="clicked()"
						>Extreem lange tekst in deze button die moet worden afgekapt maar wel in de title</Button
					>
					<div class="flex items-center gap-100">
						<Button
							icon="pencil-simple"
							title="Handig deze title"
							@click="clicked()"
						></Button>
						<Button
							icon="folder-simple"
			...
			<div class="border-spacing-200 rounded-lg border border-dotted border-purple-500 p-200">
				...
				>
				<DropDown
					v-model="dropDownValues.multiple"
					:options="options"
					placeholder="Kies hier iets"
					:multiple="true"
					:validation="{ required: true }"
					help="Maak een keuze"
					>Eenvoudig Multiple</DropDown
				>
				<DropDown
					v-model="dropDownValues.simpleIcon"
					:options="iconOptions"
					placeholder="Kies hier iets"
					:validation="{ required: true }"
					help="Maak een keuze"
					>Met Ikonen en Labels</DropDown
				>
				<DropDown
					v-model="dropDownValues.multipleIcons"
					:options="iconOptions"
					placeholder="Kies hier iets"
					:multiple="true"
					:validation="{ required: true }"
					help="Maak een keuze"
					>Multiple Ikonen en Labels</DropDown
				>
				<DropDown
					:options="userOptions"
					:transformer="dropDownData.transformUser"
					placeholder="Selecteer een user"
					:filtered="true"
					:validation="{ required: true }"
					help="Users"
					>Userlist</DropDown
			...
			<div class="border-spacing-200 rounded-lg border border-dotted border-purple-500 p-200">
				...
				<TextField
					...
					help="Hier dus je achternaam"
					>Achternaam</TextField
				>
				<TextField
				...
				>
				<TextArea
					placeholder="Type veel"
					help="Echt lange tekst kan hier"
				...
				<TextFieldAutoComplete
					placeholder="start typen"
					:options="options"
					>Autocomplete simple</TextFieldAutoComplete
				>
			</div>
			...
			<div class="border-spacing-200 rounded-lg border border-dotted border-purple-500 p-200">
				...
				<div class="flex flex-col gap-100">
					...
					<Button
						v-context-menu="(evt: any) => openMenu(evt, items2)"
						icon="check-circle"
						variant="primary"
						title="this is a tooltip"
						@click="clicked()"
					...
					<TextField
						placeholder="Korte tekst"
						:validation="{ maxLength: 10 }"
						help="Hier dus een korte tekst"
						>Korte</TextField
					>
					<TextArea
						placeholder="Lange tekst"
		...
		<div class="my-200 border-spacing-200 rounded-lg border border-dotted border-purple-500 p-200">
			...
			<ValidatedForm v-slot="{ isValidated }">
				...
				<TextField
					...
					placeholder="Geef getal"
					:validation="{ required: true, isNumber: true, minValue: 2, maxValue: 20 }"
					help="Hoe oud ben je?"
					>Leeftijd</TextField
				>
				<TextField
					v-model="formValues.specific"
					placeholder="Type Yes or No"
					:validation="{ custom: mustBeYesOrNo() }"
					>Yes or No</TextField
				>
				<TextArea
				...
				<div class="mb-200">
					<RadioGroup
						...
						:validation="{ required: true, custom: customRadioValidation() }"
						class="radiogroup-in-form"
					>
						<Label>Keuze:</Label>
						<Radio
							v-model="formValues.radio"
...
<script setup lang="ts">
	...
	const radioModel = defineModel<string>('');
	...
	onMounted(() => {
		document.addEventListener('context-menu-select', ((e: CustomEvent) => {
			const item = e.detail.item;
			if (item.label === 'Open1') {
				// eslint-disable-next-line no-console -- debug logging for design page
