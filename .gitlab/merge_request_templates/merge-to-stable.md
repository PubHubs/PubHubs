...
- If you do the merge together with a collegue, one of you should do the tests on an mobile phone and the other on a desktop.
    ...
    - [ ] Please check that the following works on https://main.pubhubs.ihub.ru.nl/client :
        - [ ] Test basic pubhubs functionality
            - [ ] Open two (private) browser windows and:
                ...
                - [ ] Register a _new_ user, with fresh Yivi credentials.
    ...
    - [ ] Check that all servers start up and are reachable https://uptime.pubhubs.net/dashboard/29
    - [ ] Check that the following works on https://app.pubhubs.net :
        ...
        - [ ] Test basic pubhubs functionality (see above)
            - Note for registering a _new_ user, with a fresh email address: If possible, tips are if you have a gmail account you can add +<date> before the '@' and add a card for that, if you have a mail domain with a catch-all you can create a new mail address and add a card for that.
