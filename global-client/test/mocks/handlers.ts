...
export const handlers = [
		...
		const data: PHCWelcomeResp = {
			Ok: {
				constellation: {
					...
					global_client_url: 'http://testdomain2',
	...
	http.get('http://testdomain2/.ph/user/state', () => {
	...
	http.get('http://testdomai2/.ph/user/obj/by-hash/globalSettingsHash/globalSettingsHmac', async () => {
	...
	http.get('http://testdomain2/logout', () => {
		localStorage.removeItem('PHauthToken');
		localStorage.removeItem('UserSecret');
		return new HttpResponse(null, { status: 200 });
	}),

	http.post('http://testdomain2/.ph/user/obj/by-handle/usersecret', async ({ request }) => {
	...
	http.post('http://testdomain2/.ph/user/obj/by-hash/usersecret/userSecretHash', async ({ request }) => {
		const body = await request.arrayBuffer();
		encryptedUserSecret = body;
		return HttpResponse.json({ Ok: { Stored: { object_details: { hash: 'userSecretHash', hmac: 'userSecretHmac', size: 300 } } } }, { status: 200 });
	}),

	http.post('http://testdomain2/.ph/user/obj/by-handle/usersecretbackup', async ({ request }) => {
		const body = await request.arrayBuffer();
	...
	http.post('http://testdomain2/.ph/user/obj/by-hash/usersecretbackup/userSecretBackupHash', async ({ request }) => {
		const body = await request.arrayBuffer();
		encryptedUserSecretBackup = body;
		return HttpResponse.json({ Ok: { Stored: { object_details: { hash: 'userSecretBackupHash', hmac: 'userSecretBackupHmac', size: 300 } } } }, { status: 200 });
	}),

	http.get('http://testdomain2/.ph/user/obj/by-hash/userSecretHash/userSecretHmac', () => {
	...
	http.get('http://testdomain2/.ph/user/obj/by-hash/userSecretBackupHash/userSecretBackupHmac', () => {
