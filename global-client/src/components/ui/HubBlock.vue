<template>
	<div v-if="hub" class="bg-background relative flex h-60 w-full max-w-full flex-col overflow-hidden rounded-xl shadow-md hover:cursor-pointer" @click="enterHub(hub)">
		...
		<div v-if="showDescription" class="global-preview bg-background absolute right-0 top-0 z-30 h-full max-h-60 w-full overflow-y-auto rounded-xl p-4">
		...
		<div class="flex h-min items-start gap-4 px-4 py-2">
			...
			<div class="flex h-full w-full max-w-full flex-col justify-center gap-2 overflow-hidden pb-2 pt-1 text-left">
				...
				<div class="h-16">
					...
					<Pre v-model="summary" class="font-body max-w-[calc(100%_-_2em)] hyphens-auto whitespace-pre-line break-words" :class="isMobile ? 'line-clamp-3 text-xl' : 'line-clamp-2'">{{ summary }}</Pre>
				</div>
			</div>
		</div>
	</div>
