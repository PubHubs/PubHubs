<template>
	<OnboardingTemplate>
		<template #column1>
			...
			<a href="javascript:void(0)" class="mt-8 hidden underline underline-offset-4 text-label lg:block" @click="$emit('back')">&#x27F5; {{ $t('register.step_previous') }}</a>
		</template>
		<template #column2>
			<div class="flex w-3/4 flex-col lg:w-full">
				...
				<a href="javascript:void(0)" class="flex underline underline-offset-4 text-label lg:hidden" @click="$emit('back')">&#x27F5; {{ $t('register.step_previous') }}</a>
			</div>
			<div class="lg:hidden">
