	...
	<div class="bg-surface flex h-[80px] w-full items-center px-6 py-4">
		<div class="flex h-full w-full items-center justify-between gap-16">
			...
			<div class="flex h-4 items-center justify-center gap-2">
				<p v-if="settings.language === 'nl'" class="hover:text-accent-primary cursor-pointer font-bold" @click="changeLanguage('en')">EN</p>
				<p v-else class="hover:text-accent-primary cursor-pointer font-bold" @click="changeLanguage('nl')">NL</p>
			</div>
		</div>
	</div>
</template>
