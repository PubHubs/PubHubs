	...
	<draggable :list="global.pinnedHubs" :item-key="'hubId'">
		<template #item="{ element, index }">
			<router-link :to="{ name: 'hub', params: { id: element.hubId } }" v-slot="{ isActive }">
				<HubIcon v-if="global.loggedIn || element.hubId == hubs.currentHubId" :hub="hubs.hub(element.hubId)" :active="isActive" :pinned="true" @remove="global.removePinnedHub(index)" @click="toggleMenu.toggleMenu()"></HubIcon>
			</router-link>
		</template>
	</draggable>
</template>
