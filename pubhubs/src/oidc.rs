...
mod http {
    ...
    impl From<redirect_uri::Response> for Response {
        fn from(fp: redirect_uri::Response) -> Self {
            Response::FormPost(fp)
        }
    }

    /// Enumerates the errors returned by oidc via http.
    #[derive(Debug, PartialEq)]
    pub enum Error {
...
impl<H: Handler> Oidc for OidcImpl<H> {
    ...
    fn handle_auth(&self, req: H::Req) -> H::Resp {
        macro_rules! http_error {
            ($param:tt) => {
                H::Resp::from(http::Error::$param.into())
            };
        }
        ...
        let err_resp = |error_type: redirect_uri::Error| -> H::Resp {
            H::Resp::from(http::Response::FormPost(redirect_uri::Response {
        ...
        .to_handle(&self.auth_request_handle_secret))
        {
            ...
            Err(err) => {
                ...
                return err_resp(redirect_uri::Error::ServerError);
            }
        }
    }
    ...
    fn handle_token(&self, req: H::Req) -> H::Resp {
        ...
        return H::Resp::from(http::Response::IdToken(acd.id_token));
    }
}
...
mod tests {
    ...
    fn handle_auth() {
        ...
        macro_rules! http_error {
            ($param:tt) => {
                MockHttpResponse::FromOidc(http::Error::$param.into())
            };
        }
        ...
        for param in vec![
            ...
            assert_eq!(
                ...
                MockHttpResponse::FromOidc(http::Response::FormPost(redirect_uri::Response {
    ...
    fn handle_token() {
        ...
        assert_eq!(
            s.handle_token(&oidc),
            http::Response::IdToken("id_token".to_string())
        );
        ...
        // wrong method
        {
            ...
            assert_eq!(s.handle_token(&oidc), http::Error::UnsupportedMethod.into())
        }
        ...
        // wrong content type
        {
            ...
            assert_eq!(
                s.handle_token(&oidc),
                http::Error::UnsupportedContentType.into()
            )
        }
        ...
        // invalid body
        {
            ...
            assert_eq!(
                s.handle_token(&oidc),
                http::Error::MalformedRequestBody.into()
            )
        }
        ...
        // invalid grant type
        {
            ...
            assert_eq!(
                s.handle_token(&oidc),
                http::Error::UnsupportedGrantType.into()
            )
        }
        ...
        // authorization problems
        {
            ...
            assert_eq!(
                s.handle_token(&oidc),
                http::Error::MissingClientCredentials.into()
            );
            ...
            assert_eq!(
                s.handle_token(&oidc),
                http::Error::InvalidClientCredentials.into()
            );
        }
...

        {
            ...
            s.set_query();
            s.set_request();
            assert_eq!(s.handle_token(&oidc), http::Error::InvalidClientMAC.into());
        }

        {
            ...
            s.set_query();
            s.set_request();
            assert_eq!(s.handle_token(&oidc), http::Error::InvalidAuthCode.into());
        }
