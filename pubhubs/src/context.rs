...
use crate::{
    config::having_debug_default, context, data::DataCommands,  oidc,
    oidc_handler,
};
...
pub struct Main {
    ...
    pub id_token_key : ed25519_dalek::Keypair,
...
impl Main {
    pub async fn create(config: crate::config::File) -> Result<Arc<Self>> {
        ...
        let id_token_key : ed25519_dalek::Keypair =   {
                let id_token_private_key = ed25519_dalek::SecretKey::from_bytes(
                    crate::crypto::derive_secret("id-token-key", &oidc_secret).as_slice())?;
                ed25519_dalek::Keypair{
                    public: (&id_token_private_key).into(),
                    secret: id_token_private_key,
                }
        };


        let well_known_jwks_json: bytes::Bytes = serde_json::to_string_pretty(&serde_json::json!({
